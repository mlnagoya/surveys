...
- RoPE はチャネル数 (偶数) を2つずつペアにして、各ペアに対して異なる周波数で回転をかける位置エンコーディング。回転の角度はトークンの位置に依存する。
- Point RoPE は3次元空間に対応するために、チャネル数を6の倍数にして、3つずつペアに分けて点の (x, y, z) の位置に応じて異なる角度で回転をかける
